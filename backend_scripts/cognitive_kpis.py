...
def _optimal_range_score(x, lo, hi, below_falloff, above_falloff):
    """Smooth 'plateau' score: 10 inside [lo, hi], linear decay outside.

    Replaces hard step ladders so a small change in the measured rate doesn't
    cause a multi-point jump. `*_falloff` is the distance over which the score
    decays from 10 to 0 on each side of the optimal band.
    """
    if x < lo:
        return max(0.0, 10.0 - (lo - x) / max(below_falloff, 1e-6) * 10.0)
    if x > hi:
        return max(0.0, 10.0 - (x - hi) / max(above_falloff, 1e-6) * 10.0)
    return 10.0
...
def cognitive_load(report):
    ...
    Computation: combines on-screen text density with salience competition —
    how peaked the predicted gaze is (one clear focal point vs. salience
    smeared across many zones). High score = manageable load.

    NOTE: the previous version used the mean key-frame *attention* score as a
    "clutter" proxy, which inverted the metric — a compelling, high-attention
    video was scored as high cognitive load. We now read actual competition
    signals (gaze dispersion + focus + text) instead.
    """
    ...
    # Text density penalty: verbose on-screen copy competes for working memory.
    text_penalty = 0.0
    ...
    # Salience competition: a clear focal point (high peak-to-mean confidence,
    # few distinct gaze zones) keeps load low; salience spread across many zones
    # means more elements competing at once -> higher load (lower score).
    if gaze_data:
        zones = [g["zone"] for g in gaze_data]
        distinct_zones = len(set(zones))
        mean_conf = float(np.mean([g["confidence_ratio"] for g in gaze_data]))
        focus_score = min(mean_conf / 8.0, 1.0) * 6.0       # 0-6: peaked = easy to parse
        dispersion_penalty = max(0, distinct_zones - 3) * 1.0
    else:
        focus_score, dispersion_penalty, distinct_zones, mean_conf = 3.0, 0.0, 0, 0.0

    load_score = 4.0 + focus_score - text_penalty - dispersion_penalty
    ...
    return {
        ...
        "methodology": (f"Salience focus {mean_conf:.1f} across {distinct_zones} gaze zones, "
                        f"text penalty {text_penalty:.1f}. High = manageable; Low = working memory exceeded."),
...
def pattern_interrupt_strength(report):
    ...
    # 2-5 interrupts per 10s is ideal; smooth decay below or above.
    score = _optimal_range_score(interrupts_per_10s, 2.0, 5.0,
                                 below_falloff=2.5, above_falloff=6.0)
...
def attention_switching_cost(report):
    ...
    # Smooth decay on both sides (was a 5-step ladder).
    score = _optimal_range_score(transitions_per_sec, 0.3, 0.7,
                                 below_falloff=0.5, above_falloff=1.2)
