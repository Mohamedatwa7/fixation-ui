...
export default function KpiStrip({ kpis }: Props) {
  return (
    <div>
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 border border-white/10 rounded-[3px] overflow-hidden">
        {kpis.map((kpi, i) => (
          <div
            key={kpi.id}
            className={`relative group bg-panel px-4 py-6 flex flex-col gap-2.5
              hover:bg-elevated transition-colors duration-300
